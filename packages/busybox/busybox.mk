...
define busybox/build :=
	...
	if [ $(STATIC) -eq  1 ]; then
		...
		sed -i 's|CONFIG_SYSROOT=""|CONFIG_SYSROOT="$(BUILD)"|g' .config
	fi
	#+$(CROSS_MAKE_ENV) '$(MAKE)' ARCH=arm CROSS_COMPILE='$(CROSS_PREFIX)' CONFIG_PREFIX='$(HOST)/sysroot' install -j 8
	PATH=/mnt_sdb/git/experiments/exp_04/toolchain/bin:$PATH '$(MAKE)' ARCH=arm CROSS_COMPILE='$(CROSS_PREFIX)' CONFIG_PREFIX='$(HOST)/sysroot' install -j 8
endef
